...
def animate(i):
    global xs,ys,error,tn

    xs.append(tn)
    tn=tn+1
    ys.append(math.sqrt(error[1]*error[1]+error[0]*error[0]))
...
def Jacobian_Callback(data):
    ...
    jac=jacobian_function.calc_jack(joint_states[0],joint_states[1],joint_states[2],joint_states[3],joint_states[4],joint_states[5])
    #jac_inverse = jac_array.reshape((6,7))
    #print (jac_inverse)
    jac_inverse=jac
    #jac_inverse = np.linalg.pinv(jac)
    vel_eee= Vcam_to_base.Vc_2_Base(vel_ee,joint_states)
    #vel_eee=[[0],[0],[0],[0],[0],[1]]
    vel_joints = np.matmul(jac_inverse, vel_eee)
...
def Image_Depth_Callback(depth_data):
    # print("depth")
    global curr_features, centre_yellow, centre_blue, centre_green, centre_red,rgb_img
    bridge = CvBridge()
